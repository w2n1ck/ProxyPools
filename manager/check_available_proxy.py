...
'''
-----------------------------------
    FileName:     check_available_proxy
    Description:  周期检查可用性
    Author:       瓦都尅
    Date:         2019/11/1
-----------------------------------
...
class Check(ProxyManager, Thread):
    ...
    def __init__(self, queue, thread_name):
        ...
        self.queue = queue
        self.log = LogHandler('userful_proxy_check')

    def run(self):
        self.log.info('Useful Proxy Check - {} : start'.format(self.name))
        while True:
            try:
                proxy_key = self.queue.get(block=False)
            except Empty:
                self.log.info('Useful Proxy Check - {} : end'.format(self.name))
                break
            ...
            if status or proxy_obj.fail_count < FAIL_COUNT:
                ...
                self.log.info('Useful Proxy Check - {}: {} validation pass'.format(self.name, proxy_obj.proxy))
            else:
                self.log.info('Useful Proxy Check - {}: {} validation fail'.format(self.name, proxy_obj.proxy))
                self.client.delete(proxy_obj.proxy)
            self.queue.task_done()


def available_queue_check():
    proxy_queue = Queue()
    pm = ProxyManager()
